    ...
    if np.any(true_labels!=0):
        for true_fission in np.unique(true_labels)[1:]: #Positives, first label is skipped as it corresponds to background
            ...
            if len(overlapping_fissions)>0:
                ...
                pred_area = pred_labels[pred_labels == overlapping_fissions[0]].sum()
                true_area = true_labels[true_labels == true_fission].sum()
                if pred_area/true_area > 100 or pred_area/true_area < 0.01:
                    print("Very non-uniform prediction and gt event sizes. Check also visually.")
                    continue
